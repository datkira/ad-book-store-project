...
class ApiService {
  ...
  async deleteAllCart() {
    const result = await axios.delete(`${this.API_URL}/cart`)
    return result
  }
  ...
  async acceptBill(id) {
    const result = await axios.put(`${this.API_URL}/bill/enable/${id}`)
    return result
  }

  async deleteBill(id) {
    const result = await axios.delete(`${this.API_URL}/bill/${id}`)
    return result
  }
